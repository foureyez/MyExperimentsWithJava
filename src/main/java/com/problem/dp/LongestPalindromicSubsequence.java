...
/**
 * @author foureyez Given a sequence, find the length of the longest palindromic
 *         subsequence in it.<br>
 *         If the given sequence is BBABCBCAB, then the output should be 7 as
 *         BABCBAB is the <br>
 *         longest palindromic subseuqnce in it. BBBBB and BBCBB are also
 *         palindromic subsequences <br>
 *         of the given sequence, but not the longest ones. <br>
 *         // Every single character is a palindrome of length 1 L(i, i) = 1 for
 *         all indexes i in given sequence // IF first and last characters are
 *         not same If (X[i] != X[j]) L(i, j) = max{L(i + 1, j),L(i, j - 1)} //
 *         If there are only 2 characters and both are same Else if (j == i + 1)
 *         L(i, j) = 2 // If there are more than two characters, and first and
 *         last // characters are same Else L(i, j) = L(i + 1, j - 1) + 2
 */
public class LongestPalindromicSubsequence {
	...
	/**
	 * Dynamic Programming Approach (Using Memoization for storing the result of
